...
def train(model, train_loader, test_loader, trh, tsh, epochs=(total_epochs-resume_epoch), output_freq=100):
    """Train the model.
    Args:
...
def save_model(epoch, model, scheduler, train_history, test_history, model_dir=None):
    ...
    filename=f'model_muons_final_@epoch_{epoch}.pt'
...
def load_model(model_dir=None, filename=None):
    ...
    # get learning rate at loading epoch

    lr = checkpoint['optimizer_state_dict']['param_groups'][0]['lr']
    
    return model, scheduler_present_in_checkpoint, flow_lr, epoch, train_history, test_history, lr



if __name__=='__main__':

    train_ds = H5Dataset(['../muonData/amuons1.hdf5', '../muonData/amuons2.hdf5', '../muonData/amuons3.hdf5', 
                            '../muonData/amuons4.hdf5', '../muonData/amuons5.hdf5', '../muonData/amuons6.hdf5', '../muonData/amuons8.hdf5', '../muonData/amuons9.hdf5'])
    ...
    flow, _, _, _, rtrh, rtsh, resume_lr = load_model("./saves/", f"model_muons_final_@epoch_{resume_epoch}.pt")

    total_params = sum(p.numel() for p in flow.parameters() if p.requires_grad)
    print(total_params)
    print(len(train_ds))
    print(len(train_loader.dataset))
    optimizer = torch.optim.Adam(flow.parameters(), lr=resume_lr)
    flow.to(device)

    trh, tsh = train(flow, train_loader, test_loader, rtrh, rtsh)
