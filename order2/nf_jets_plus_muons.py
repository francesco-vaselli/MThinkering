...
class MyDataset(Dataset):
  ...
  def __init__(self,file_name, start, stop):
    ...
    tvars = ["MJet_area", "MJet_btagCMVA", "MJet_btagCSVV2", "MJet_btagDeepB", "MJet_btagDeepC", "MJet_btagDeepFlavB", "MJet_btagDeepFlavC",
        "MJet_etaMinusGen", "MJet_bRegCorr", "MJet_massRatio", "MJet_nConstituents", "MJet_phiMinusGen", 
        "MJet_ptRatio", "MJet_puIdDisc", "MJet_qgl", "MJet_muEF", "MJet_nMuons"] 
    df_r = df_r[df_r.MJet_massRatio < 7]
    df_r = df_r[df_r.MJet_massRatio > 0]
    df_r = df_r[df_r.MJet_ptRatio < 3]
    df_r = df_r[df_r.MJet_ptRatio > 0]
    maskB = (df_r["MJet_btagDeepB"]<0) 
    maskC = (df_r["MJet_btagDeepB"]<0) 
    maskCSV = (df_r["MJet_btagDeepB"]<0) 
    maskQGL = (df_r["MJet_qgl"]<0) 
    df_r.loc[maskB, "MJet_btagDeepB"] = -0.1
    df_r.loc[maskC, "MJet_btagCSVV2"] = -0.1
    df_r.loc[maskCSV, "MJet_btagDeepC"] = -0.1
    df_r.loc[maskQGL, "MJet_qgl"] = -0.1
    x = df_r.loc[:, tvars].values
    y= df_r.iloc[:,0:14].values
...
def save_model(epoch, model, scheduler, train_history, test_history, model_dir=None):
    ...
    filename=f'model_jets_plus_muons_@epoch_{epoch}.pt'
