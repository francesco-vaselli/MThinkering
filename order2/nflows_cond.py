# events generation through normalizing spline flows
...
class MyDataset(Dataset):
  ...
  def __init__(self,file_name, start, stop):
    df = pd.read_hdf(file_name, start=start, stop=stop)
    df = df[~df.isin([np.nan, np.inf, -np.inf]).any(1)].astype('float32')
    int_cols = ["MJet_cleanmask", "MJet_hfsigmaEtaEta", "MJet_hfsigmaPhiPhi", "MJet_hadronFlavour", "MJet_hfadjacentEtaStripsSize", "MJet_hfcentralEtaStripSize", "MJet_jetId", 
            "MJet_nElectrons", "MJet_nMuons", "MJet_partonFlavour", "MJet_puId"]
    df = df.drop(columns=int_cols)
...
if __name__=='__main__':
    ...
    train_ds = MyDataset(file_name="../jetData/MjetData.h5", start=0, stop=1.024e6+18)
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, pin_memory=True,  num_workers=9)
    test_ds = MyDataset(file_name="../jetData/MjetData.h5", start=1.024e6+19, stop=1.024e6 + 1.024e4+19)
    ...
    flow = create_NDE_model(15, 6, 15, param_dict)
