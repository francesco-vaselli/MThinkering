...
def loss_function(recon_x, x, mu, logvar, logscale):
    ...
    recon_loss = loss(recon_x, x.view(-1, 784))
    ...
    # log_pxz = dist.log_prob(x.view(-1, 784))
    # GLL = log_pxz.sum()
...
def test(epoch):
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(test_loader):
            data = data.to(device)
            recon_batch, mu, logvar = model(data)
            test_loss += loss_function(recon_batch, data, mu, logvar, model.log_scale).item()
            if i == 0:
                n = min(data.size(0), 8)
